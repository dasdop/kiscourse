...
# ==========================================
# 3. 유틸리티 로직 (🔥 KeyError: '과목명' 방어 로직 추가)
# ==========================================
def mask_name(name):
...
def load_and_filter_data(semester):
    # 구글 시트를 못 읽거나 '과목명'이 없을 때를 대비한 비상용 데이터
    ...
    try:
        def process(url, sem):
            ...
            df.columns = df.columns.str.strip() # 혹시 모를 띄어쓰기 공백 제거
            if '학기' in df.columns: 
        ...
        # '과목명' 컬럼이 없으면 안전하게 비상용 데이터로 대체
        if '과목명' not in df_11.columns: df_11 = dummy_df[dummy_df['학기'] == semester.replace("학기", "")]
        if '과목명' not in df_12.columns: df_12 = dummy_df[dummy_df['학기'] == semester.replace("학기", "")]
        return df_11, df_12
...
def draw_styled_dataframe(df):
    # 🔥 AttributeError: applymap 에러 해결! (최신 Pandas 호환)
    styler = df.style
    if hasattr(styler, "map"):
        styled_df = styler.map(highlight_timetable) # 최신 버전
    else:
        styled_df = styler.applymap(highlight_timetable) # 구 버전
...
if st.session_state.page == 'login':
    ...
    if st.button("로그인"):
        ...
        else: st.error("올바른 학번을 입력하세요 (10... 또는 11...)")
...
# [B] 대시보드
    ...
    if st.session_state.user_id != "admin" and st.session_state.app_status == "준비중":
        st.error("🔒 관리자가 수강신청을 오픈할 때까지 대기해주세요. (시뮬레이션은 가능합니다)")
    ...
    with c4:
        if st.button("🎮 시뮬레이션(연습)", use_container_width=True): st.session_state.page = "simulation"; st.rerun()
...
# [C] 정규 수강신청 폼
    ...
    st.info(f"💡 {st.session_state.grade}은 반드시 **{req_count}개**를 선택해야 합니다.")
        ...
        if st.form_submit_button("신청서 제출"):
            if len(selected) != req_count: st.error(f"❌ 정확히 {req_count}개를 골라주세요!")
                ...
                new_data = {
                    ...
                    '학년': st.session_state.grade, '희망계열': track, '신청과목': ",".join(selected)
                }
...
# [D] 시뮬레이션 (학생 연습용)
    ...
    st.title("🎮 수강신청 시뮬레이션 (연습모드)")
    st.info("이곳에서 선택한 과목은 실제 서버에 저장되지 않습니다. 시간표 충돌 여부를 미리 확인하세요!")
    
    gr = st.radio("테스트할 학년", ["11학년", "12학년"]) if st.session_state.user_id == "admin" else st.session_state.grade
        
    req_count = 7 if gr == "11학년" else 8
    df_11, df_12 = load_and_filter_data(st.session_state.selected_semester)
    available = df_11['과목명'].unique().tolist() if gr == "11학년" else df_12['과목명'].unique().tolist()
            ...
            if c in MASTER_TIMETABLE:
                for day, p in MASTER_TIMETABLE[c]:
                    if not (day == "금" and p in ["7교시", "8교시"]): 
                        ...
                        if current_val == "": timetable.at[p, day] = c
                        else: timetable.at[p, day] = f"💥충돌 ({current_val} / {c})"
                        
        draw_styled_dataframe(timetable) # 🔥 에러 방지 함수 사용
    ...
    if os.path.exists(result_file):
        ...
        if not my_res.empty:
            ...
            for c in my_courses:
                if c in MASTER_TIMETABLE:
                    for day, p in MASTER_TIMETABLE[c]:
                        if not (day == "금" and p in ["7교시", "8교시"]): timetable.at[p, day] = c
...
# [G] 관리자 전용 데이터 관리
    ...
    st.title("⚙️ 관리자 전용 제어 및 추출")
    sem = st.session_state.selected_semester
    
    st.warning("⚠️ 이곳의 버튼들은 실제 서버 데이터에 즉시 반영되므로 주의해서 사용하세요.")
