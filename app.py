...
GROUP_MAP = {
    "A그룹": ["물리학", "미적분 II", "심화 국어", "프레젠테이션 화법"],
...
MASTER_TIMETABLE = {
    ...
    "스포츠 문화": [("월", "3교시"), ("목", "3교시")],
    "창의적 사고 설계": [("수", "2교시")],
    "심화 영어": [("월", "2교시"), ("목", "2교시")], # 12학년
    "진로 활동": [("월", "3교시"), ("목", "3교시")]  # 12학년
}
...
def draw_styled_dataframe(df):
    # 🔥 Pandas 버전 호환(applymap -> map) 에러 해결 로직
    styler = df.style
...
# [B] 대시보드
    ...
    if st.session_state.user_id != "admin" and st.session_state.app_status == "준비중":
        st.error("🔒 관리자가 수강신청을 오픈할 때까지 대기해주세요. (시뮬레이션은 언제나 가능합니다)")
...
# [D] 시뮬레이션 (학생 연습용)
    ...
    st.info("이곳에서 선택한 과목은 서버에 저장되지 않습니다. 시간표 충돌 여부를 미리 확인하세요!")
    ...
    if st.button("시간표 그리기"):
        ...
        if not_found:
            st.warning(f"⚠️ 다음 과목은 시간표 데이터(MASTER_TIMETABLE)에 등록되지 않아 표에 나타나지 않습니다: {', '.join(not_found)}\n\n(관리자에게 추가를 요청하세요!)")
