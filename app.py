...
def load_and_filter_data(semester):
    try:
        def process(url, sem):
            ...
            if '학기' in df.columns: df = df[df['학기'].str.contains(sem.replace("학기", ""), na=False)]
        ...
        return df[df['학기'] == semester.replace("학기", "")], df[df['학기'] == semester.replace("학기", "")]
...
# [C] 정규 수강신청 폼 (🔥 끊겼던 부분 복구 및 완료)
    ...
    with st.form("apply_form"):
        selected = st.multiselect(f"수강 희망 과목 (최대 {req_count}개)", available, max_selections=req_count)
...
# [D] 시뮬레이션 (학생 연습용)
    ...
    if st.session_state.user_id == "admin":
        gr = st.radio("테스트할 학년", ["11학년", "12학년"])
    else:
        gr = st.session_state.grade
