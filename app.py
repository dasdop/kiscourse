...
# [B] 메인 대시보드
    ...
    with col2:
        if st.button("📊 배정 결과 및 시간표", use_container_width=True): st.session_state.page = "result"; st.rerun()
    with col3:
    ...
    with col4:
        ...
        if st.button("⚙️ 관리 시스템" if is_admin else "🔔 알림함", use_container_width=True):
            st.session_state.page = "admin_page" if is_admin else "noti"; st.rerun()
...
# [C] 수강신청 페이지
    ...
    cur_grade = 11 if u_id.startswith("10") else 12
    ...
    if target_list is not None:
        ...
        with st.form("apply_form"):
            ...
            my_track = st.selectbox("희망 계열(우대)", TRACKS) if cur_grade == 12 else "공통"
...
# [E] 과목 거래소
    ...
    st.title("🤝 과목 거래소")
    if st.button("⬅️ 메인으로"): st.session_state.page = "dashboard"; st.rerun()
    t1, t2 = st.tabs(["📜 게시판", "✍️ 거래 올리기"])
    with t2:
        with st.form("trade_post"):
            give, take, note = st.text_input("줄 과목"), st.text_input("받을 과목"), st.text_area("메모")
            if st.form_submit_button("등록"):
                pd.DataFrame([{'시간':datetime.now().strftime('%m-%d %H:%M'), '이름':st.session_state.user_name, '줄':give, '받':take, '메모':note}]).to_csv('trade_posts.csv', mode='a', header=not os.path.exists('trade_posts.csv'), index=False, encoding='utf-8-sig')
                st.success("등록 완료!"); st.rerun()
    with t1:
        tdf = get_csv_df('trade_posts.csv')
        if tdf is not None:
            for _, r in tdf.iloc[::-1].iterrows():
                with st.expander(f"🔄 {r['줄']} ↔ {r['받']} ({r['이름']})"):
                    st.write(f"💬 {r['메모']}")
        else: st.info("게시글이 없습니다.")
...
# [F] 관리자 전용 시뮬레이션 및 배정
    ...
    def run_simulation(student_count, req_count, subject_list):
        if not subject_list: return None, 0
        sim_results = []
        total_sat = 0
        for i in range(student_count):
            req = random.sample(subject_list, min(req_count, len(subject_list)))
            assigned = random.sample(req, int(len(req)*0.85)) # 85% 성공 가정
            sat = (len(assigned)/req_count)*100
            total_sat += sat
            sim_results.append({"학번": f"SIM-{i:03d}", "만족도": f"{sat:.1f}%", "상태": "완료"})
        return pd.DataFrame(sim_results), total_sat / student_count

    c1, c2 = st.columns(2)
    with c1:
        if st.button("📊 11학년 시뮬레이션 가동"):
            df, rate = run_simulation(170, 7, list_11)
            st.metric("평균 만족도", f"{rate:.1f}%"); st.dataframe(df, height=200)
    with c2:
        if st.button("📊 12학년 시뮬레이션 가동"):
            df, rate = run_simulation(170, 8, list_12)
            st.metric("평균 만족도", f"{rate:.1f}%"); st.dataframe(df, height=200)
    ...
    if st.button("전체 학생(students_data.csv) 실제 배정 시작", type="primary"):
        if os.path.exists('students_data.csv'):
            ...
            sdf['확정과목'] = sdf['신청과목'] # 실제 알고리즘 로직이 들어갈 자리
            sdf['탈락과목'] = "없음"
            sdf.to_csv('final_results.csv', index=False, encoding='utf-8-sig')
            st.success("배정이 완료되었습니다! 학생들이 결과를 확인할 수 있습니다.")
